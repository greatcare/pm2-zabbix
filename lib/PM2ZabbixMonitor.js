 ...
 * @param {ZabbixDataProvider} dataProvider - The interface to Zabbix. Used for sending data items.
 *        It is also populated with discovery data (the process list) initially (at start time), even if monitor mode is off.
 * @param {Object} [options] - Settings for configuring the run-time behaviour of the monitor.
 * @param {boolean} [options.monitor=false] - Whether updates should be listened to and sent periodically.
 *        By default, only the initial state is loaded and the discovery data is populated
 *        (i.e. no data gets actually sent to Zabbix). The "non-monitor" mode may be used for building one-time discovery scripts.
 * @param {boolean} [options.debug] - Whether log messages should be printed to the console.
 * @param {number} [options.processListInterval=15000] - The interval (ms) between sending entire process lists to Zabbix.
 *        This does not impact process status change updates, which are sent immediately. Meaningless if "monitor" is not enabled.
...
PM2ZabbixMonitor.prototype._initListeners = function _initListeners() {
    ...
    self._tracker.on('processStateChanged', (changeEvent) => {
        ...
        const statusKey = self.getDataKey(changeEvent.processID, changeEvent.newState, 'status');
        const statusObject = {};
        statusObject[statusKey] = changeEvent.newState.status;
        self._dataProvider.send(statusObject).done(() => {
            self._logger.debug({
                event: 'PM2ZabbixMonitor#processUpdateSent',
                processID: changeEvent.processID,
                newState: changeEvent.newState
            }, 'Real-time status update sent for process %s', changeEvent.processID);
        }, (error) => {
            self._logger.error({
                event: 'PM2ZabbixMonitor#processUpdateSendingError',
                error: error,
                processID: changeEvent.processID,
                newState: changeEvent.newState
            }, 'Real-time status update sending failed for process %s: %s', changeEvent.processID, error.message);
        });
    });
};
 ...
 * @param {string} dataItem - Which aspect of information about the process's state should be provided.
 *        Separate keys are supposed to exist server-side for the CPU usage, memory, status, etc.
...
PM2ZabbixMonitor.prototype.getDataKey = function getDataKey(processID, processState, dataItem) {
    return 'pm2.processes[' + this._escape(processID) + ',' + this._escape(dataItem) + ']';
};

/**
 * Escape the given text according to zabbix_sender man page rules
 * @param text The text to be escaped
 * @return {string} The escaped text
 * @private
 */
PM2ZabbixMonitor.prototype._escape = function escape(text) {
    let escapedText = text;

    if (escapedText.match(/[\\"]/)) {
        escapedText = escapedText.replace(/([\\"])/, '\\$1');
    }
...
PM2ZabbixMonitor.prototype.start = function start() {
    ...
    return self._tracker.start().then(() => {
        // Load the process list and add them as discovery items.
        const processMap = self._tracker.getCachedProcessMap();
        // Log the process map:
        self._logger.debug({ event: 'PM2ZabbixMonitor#processMapGenerated', processMap: processMap }, 'Process map generated');
        Object.keys(processMap).forEach((processID) => {
            self._dataProvider.addDiscoveryItem({
                '{#PROCESS_ID}': processID,
                '{#PROCESS_NAME}': processMap[processID].name,
                '{#PROCESS_MAX_MEMORY}': self.getMaxMemoryFromNodeParams(processMap[processID].nodeParams) * 1000 * 1000,
                '{#PROCESS_MAX_MEMORY_FORMATTED}': self.getMaxMemoryFromNodeParams(processMap[processID].nodeParams).toString() + 'MB',
            });
        });
    }).then(() => {
        if (self._options.monitor) {
            setInterval(() => {
                self.sendProcessList().done(() => {
...
PM2ZabbixMonitor.prototype.getMaxMemoryFromNodeParams = function getMaxMemoryFromNodeParams(params) {
    const _params = params || [];
    const memory = _params.map((p) => p.match(/max-old-space-size=([0-9]+)/)).filter((p) => p || false);
    if (memory && memory[0] && memory[0][1]) {
        return memory[0][1];
    }
    return 1400;
};
...
PM2ZabbixMonitor.prototype.sendProcessList = function sendProcessList() {
    ...
    return self._tracker.getProcessMap().then((processMap) => {
        self._logger.trace({ event: 'PM2ZabbixMonitor#gotProcessMap', processMap: processMap });
        const dataObject = {};
        Object.keys(processMap).forEach((processID) => {
            const processState = processMap[processID];
...
PM2ZabbixMonitor.prototype.sendPM2Status = function sendPM2Status() {
    ...
    return self._tracker.getPM2State().then((processState) => {
